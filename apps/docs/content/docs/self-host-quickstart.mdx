...
This page walks you through running the Multica **server** (backend + frontend) on your own machine or server with Docker. PostgreSQL is remote for deployment environments and is configured through `DATABASE_URL`.
...
- **Remote PostgreSQL 17 with pgvector**, reachable from the server
...
2. Require `.env` to contain a remote `DATABASE_URL`
3. Pull the official Docker images (Multica backend, Multica frontend)
4. Bring up the backend and frontend using `docker-compose.selfhost.yml`
5. Wait until the backend's `/health` endpoint is ready
