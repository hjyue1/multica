...
这一页带你用 Docker 把 Multica 的**服务器**（后端 + 前端）跑在自己的机器或服务器上。部署环境的 PostgreSQL 统一使用远程数据库，通过 `DATABASE_URL` 配置。
...
- **Git** 可选（推荐——可以拉源码）
- **远程 PostgreSQL 17 + pgvector**，并且服务器能访问
...
cd multica
cp .env.example .env
# 编辑 .env，把 DATABASE_URL 改成远程 PostgreSQL 连接串。
make selfhost
```
...
2. 要求 `.env` 中配置远程 `DATABASE_URL`
3. 拉取官方 Docker 镜像（Multica backend、Multica frontend）
4. 用 `docker-compose.selfhost.yml` 启动后端和前端
5. 等后端 `/health` 端点准备就绪
