...
# 所有部署环境都使用远程 PostgreSQL；不要在 VPS 上启动本地数据库。
DATABASE_URL=postgres://<user>:<password>@<remote-postgres-host>:5432/<database>?sslmode=require
...
1. 使用 Docker Compose 启动 Backend、Frontend；
2. 从当前 checkout 构建 `multica-backend:dev` 和 `multica-web:dev`；
3. Backend 通过 `.env` 中的远程 `DATABASE_URL` 连接 PostgreSQL；
4. Backend 启动时自动执行数据库迁移；
5. 服务异常退出后由 Docker 自动重启。
...
数据库必须使用远程 PostgreSQL。裸源码部署也沿用同一个原则，`.env` 中配置远程连接串：
...
数据库是远程 PostgreSQL，优先使用数据库服务商或公司统一运维提供的备份策略。需要在 VPS 上临时导出时，可以使用 PostgreSQL 镜像执行 `pg_dump`：
...
恢复前建议先停 backend，并按远程 PostgreSQL 的恢复流程执行，避免写入冲突。
...
Backend 容器启动时会自动对远程数据库执行迁移。升级前先做数据库备份。
